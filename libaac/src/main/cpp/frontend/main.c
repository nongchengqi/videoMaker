...
int main(int argc, char *argv[]) {
    ...
    if (bitRate) {
        ...
        fprintf(stderr, "Average bitrate: %d kbps/channel\n",
                (bitRate + 500) / 1000);
    } else fprintf(stderr, "Quantization quality: %ld\n", quantqual);
...
Java_com_evan_aac_AACEncoder_encodePcmToAAC(JNIEnv *env, jobject thiz, jstring in_path,
                                              jstring out_path, jint bit, jint sample_rate,
                                              jint channel) {
    const char *outPath = (*env)->GetStringUTFChars(env, out_path, 0);
    const char *inPath = (*env)->GetStringUTFChars(env, in_path, 0);
    char *channelCount = malloc(2);
    sprintf(channelCount, "%d", channel);
    char *bitWidth = malloc(3);
    sprintf(bitWidth, "%d", bit);
    char *sample = malloc(6);
    sprintf(sample, "%d", sample_rate);
    const char *arg[] = {"./faac", "-X", "-B", bitWidth,
                         "-C", channelCount, "-R",
                         sample, "-o", outPath,
                         inPath};
    main(11, (char **) arg);
